...
前提：本机已安装 **Node.js（LTS）**、**pnpm**（[安装 pnpm](https://pnpm.io/installation)）。

本仓库在 **`clawx/`** 目录内嵌了 **ClawX** 源码（与 [ValueCell-ai/ClawX](https://github.com/ValueCell-ai/ClawX) 同源定制）。

```bash
git clone https://github.com/liuxinyao123/moss-ai.git
cd moss-ai
npm run bootstrap    # 或: ./bootstrap.sh  — 会执行 根目录 npm install + clawx/ pnpm install
./start.sh
```
...
| 命令 | 含义 |
|------|------|
| `npm run bootstrap` | 根目录 `npm install` + `clawx/` 内 `pnpm install` |
| `npm start` | 仅启动主 API（3001） |
| `npm run start:clawx` | `./start.sh`（API + ClawX） |
| `npm run start:engine` | 可选实验进程，勿与主 API 同占 3001 |
| `npm run test` | Vitest |
| `npm run icons` | 从 `assets/branding/dsclaw-icon.svg` 生成 PNG（需 `sharp`） |
