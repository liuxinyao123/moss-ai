...
function resolveSkillDisplayName(skill: Skill, t: TFunction<'skills'>): string {
  const keyCandidates = [
    (skill.slug || '').trim(),
    (skill.id || '').trim(),
  ].filter(Boolean);
  for (const key of keyCandidates) {
    const localized = t(`displayName.${key}`, { defaultValue: '' }) as unknown as string;
...
function SkillDetailDialog({ skill, isOpen, onClose, onToggle, onUninstall, onOpenFolder }: SkillDetailDialogProps) {
  ...
  const displayName = resolveSkillDisplayName(skill, t);
...
export function Skills() {
  ...
  const applySkillToExpert = useCallback(async (skill: Skill) => {
    try {
      const displayName = resolveSkillDisplayName(skill, t);
      const newAgentName = `技能专家：${displayName}`;
      const created = await hostApiFetch<{ success?: boolean; createdAgentId?: string; error?: string }>(
        '/api/agents',
        {
          method: 'POST',
          body: JSON.stringify({ name: newAgentName, inheritWorkspace: true }),
        }
      );
      const agentId = String(created?.createdAgentId || '').trim();
      if (!agentId) {
        throw new Error(created?.error || '创建专家智能体失败（未返回 agentId）');
      }
      await hostApiFetch<{ success: boolean; error?: string }>('/api/skills/skill-to-expert', {
        method: 'POST',
        body: JSON.stringify({
          agentId,
          skill: {
      ...
      trackUiEvent('skills.skill_to_expert', { agentId, skillId: skill.id, slug: skill.slug });
      toast.success(t('toast.skillToExpertSuccess', { defaultValue: '已转为技能专家（人设已注入）' }));
    } catch (err) {
      toast.error(t('toast.skillToExpertFailed', { defaultValue: '转专家失败' }) + ': ' + String(err));
    }
  }, [currentAgentId, t]);
  ...
  return (
    <div className="flex flex-col -m-6 h-[calc(100vh-2.5rem)] overflow-hidden bg-[#f3f0e8] text-foreground dark:bg-background">
      <div className="w-full max-w-6xl mx-auto flex flex-col h-full p-6 md:p-8 pt-8 md:pt-10">

        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-start justify-between mb-5 shrink-0 gap-4">
          <div>
            <h1 className="text-[22px] md:text-[26px] font-semibold tracking-tight mb-1">
              {t('title')}
            </h1>
          ...
          <div className="flex items-center gap-3 md:mt-2">
            {hasInstalledSkills && (
              <button
                ...
                className="hover:bg-white/50 dark:hover:bg-white/5 transition-colors shrink-0 text-[13px] font-medium px-4 h-9 rounded-xl border border-black/10 dark:border-white/10 flex items-center justify-center text-foreground/80 hover:text-foreground bg-white/40 shadow-sm"
              >
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between pb-3 mb-4 shrink-0 gap-3">
          <div className="flex items-center flex-wrap gap-3">
            <div className="relative w-[360px] max-w-[75vw]">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input
                placeholder={t('search')}
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-9 bg-white/70 border-black/10 shadow-sm rounded-xl"
              />
              {searchQuery && (
                <button
                  type="button"
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={cn(
                  'px-3 h-9 rounded-xl text-[13px] font-medium border shadow-sm transition-colors',
                  selectedSource === 'all'
                    ? 'bg-white/70 border-black/10 text-foreground'
                    : 'bg-white/30 border-black/5 text-muted-foreground hover:text-foreground hover:bg-white/50'
                )}
              >
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              ...
              className="h-9 text-[13px] font-medium rounded-xl px-3 border-black/10 bg-white/40 hover:bg-white/60 shadow-sm"
            >
            ...
            <Button
              ...
              className="h-9 text-[13px] font-medium rounded-xl px-3 shadow-sm"
            >
            ...
            <Button
              ...
              className="h-9 w-9 ml-1 rounded-xl border-black/10 bg-white/40 hover:bg-white/60 shadow-sm text-muted-foreground hover:text-foreground"
              title={t('refresh')}
            >
                  ...
                  >
                    <div className="p-4">
                      <div className="flex items-start gap-3">
                        <div className="h-12 w-12 shrink-0 flex items-center justify-center text-2xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm">
                          {skill.icon || '🧩'}
                        </div>
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        <Badge variant="secondary" className="px-2 py-0.5 text-[11px] font-medium bg-black/5 dark:bg-white/10 border-0 shadow-none rounded-full">
                          {resolveSkillSourceLabel(skill, t)}
                        </Badge>
                        {skill.version ? (
                        ...
                        {skill.slug && skill.slug !== skill.name ? (
                          <span className="text-[11px] font-mono px-2 py-0.5 rounded-full border border-black/10 dark:border-white/10 text-muted-foreground bg-white/35">
                            {skill.slug}
                          </span>
                        ) : null}
                      </div>
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-7 rounded-full px-3 text-[11px] font-semibold border-black/10 bg-white/40 hover:bg-white/60 shadow-none"
                            onClick={() => applySkillToExpert(skill)}
                            title={t('actions.skillToExpert', { defaultValue: '转专家' })}
                          >
                            {t('actions.skillToExpert', { defaultValue: '转专家' })}
                          </Button>
                          <Switch
