...
const LOCAL_SKILLS_ROOT = join(ROOT, 'resources', 'skills', 'local');
...
// Also bundle local skills shipped with this repo (no network required).
if (existsSync(LOCAL_SKILLS_ROOT)) {
  const localDirs = (await $`ls -1 ${LOCAL_SKILLS_ROOT}`.quiet()).stdout
    .split('\n')
    .map((v) => v.trim())
    .filter(Boolean);
  for (const slug of localDirs) {
    ...
    const skillManifest = join(targetDir, 'SKILL.md');
    if (!existsSync(skillManifest)) {
      throw new Error(`Local skill ${slug} is missing SKILL.md after copy`);
    }
    ...
    lock.skills.push({
      ...
      version: 'local',
      repo: 'local',
      repoPath: `resources/skills/local/${slug}`,
      ref: 'local',
      commit: 'local',
    });
