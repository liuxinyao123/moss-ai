...
function getElectronApp() {
  if (process.versions?.electron) {
    try {
      ...
      const app = (electron as any)?.app as (typeof import('electron').app | undefined);
      // In some dev boot paths, `electron` can be present but `app` is not yet available.
      // Never crash path utilities in that case.
