...
async function ensureScopedChannelBinding(channelType: string, accountId?: string): Promise<void> {
  ...
  if (accountId === 'default') {
    // IMPORTANT: Do not override explicit bindings.
    // If the user has already bound `${channelType}:default` to some agent, keep it.
    const existingOwner = agents.channelAccountOwners?.[`${storedChannelType}:default`];
    if (!existingOwner && agents.agents.some((entry) => entry.id === 'main')) {
